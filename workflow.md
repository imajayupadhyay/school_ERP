...
- School Creation & Management (Phase 4, module 2):
  - Platform CRUD for school tenants. `School` model gained an `employees()` relation (for counts). Endpoints under `/api/v1/platform` (all `platform.admin`-gated, audit-logged): `GET /schools` (paginated, `?search=` name/code/city/email + `?status=` filter, returns `students_count`/`employees_count`/`users_count` via `withCount`), `POST /schools` (create tenant + first owner admin), `GET /schools/{school}` (detail + owner admins), `PUT /schools/{school}` (edit profile + status), `POST /schools/{school}/status` (lifecycle change). School is the tenant root (no `BelongsToSchool`), so route binding resolves any school.
  - `App\Services\Platform\SchoolProvisioningService` does create-with-owner in **one transaction**: insert the school (auto-generates a unique uppercase `code` from the name when none given), ensure the permission catalog exists then `AccessProvisioner::provisionSchool()` (default system roles), and create the owner admin (`role = school_admin`, linked to the provisioned owner role → resolves to `*`). Password is taken from the request or a 12-char temp password is generated and returned **once** (never stored). `setStatus()` handles lifecycle changes. Audit actions: `school.created`, `school.updated`, `school.status_changed`.
  - Requests `StoreSchoolRequest` / `UpdateSchoolRequest` (unique code, status enum, full profile fields + `admin_*` on create). Resource `Platform\PlatformSchoolResource` (full profile + counts + owner admins when loaded).
  - Web (`features/platform/schools/`): **Schools** sidebar item enabled → `/platform/schools`. List page (search + status filter + pagination, avatar tiles, student/staff counts, status pills, View/Edit row actions) reusing the admin design-system. `SchoolFormModal` (create with owner-admin section / edit), `CreatedCredentialsModal` (one-time owner sign-in details + copy + temp password), `SchoolDetailModal` (stats, status quick-actions suspend/deactivate/reactivate, profile grid, owner-admin list). Shared `StatusPill` extracted to `features/platform/components/` (dashboard now reuses it).
  - **School deletion (full cascade cleanup):** `DELETE /platform/schools/{school}` permanently removes the school and **every** record belonging to it. `App\Services\Platform\SchoolDeletionService` **auto-discovers tenant tables by the presence of a `school_id` column** (scoped to the current DB — driver-aware: `information_schema` on MySQL, `sqlite_master` in tests — so future tenant tables are cleaned automatically and nothing is missed), deletes each filtered by the exact `school_id`, and purges the two tenant tables without a `school_id` (`role_permissions` via the school's roles, `personal_access_tokens` via the school's users) — so **no other school is ever touched** and the platform super admin's own account/token survives. The whole purge runs in a transaction with FK constraints disabled (`Schema::withoutForeignKeyConstraints`, driver-agnostic) so ordering can't fail and a partial delete can't commit. A **platform-level audit** (`school.deleted`, `school_id = null`) is written after, so the record survives the purge. Safety: the controller requires `confirm_code` to equal the school code (422 otherwise) on top of the `platform.admin` gate. Web: red **Delete** row action + a **type-the-code** `DeleteSchoolModal` (lists what will be removed) + a danger-zone Delete button in the detail modal + a success banner.
  - Backend feature tests in `PlatformSchoolManagementTest.php` (7) and `PlatformSchoolDeletionTest.php` (4: purges all of one school's data across every tenant table while a second school stays fully intact, no orphaned `role_permissions`, platform `school.deleted` audit survives; wrong confirm-code 422 deletes nothing; acting super admin unaffected; school-user 403). Full suite: **162/162 passing**.
  - Verified with `php artisan test`, `npm run build` (262 modules, no TS errors; existing chunk-size warning), ESLint (clean), and live curl smokes: create w/ auto code + temp password → search → dashboard reflects new tenant; delete with wrong code → 422, correct code → 200, school + users gone, **Demo school + its 205 students untouched**, platform `school.deleted` audit written.

- Marketing enquiries / sales leads (Phase 4 addition):
  - The public homepage "Start Trial" button now opens an **enquiry form** (name, email, phone) instead of the old school-registration modal. New platform-level `enquiries` table (NOT tenant-scoped — leads exist before any school), model `Enquiry` (statuses: new/contacted/converted/closed).
  - API: public `POST /api/v1/enquiries` (throttled `throttle:10,1`, validates name/email/phone, stamps `ip_address`, status `new`). Platform (`platform.admin`-gated): `GET /platform/enquiries` (paginated, search name/email/phone, status filter, `meta.new_count`), `POST /platform/enquiries/{enquiry}/status` (audited `enquiry.status_changed`), `DELETE /platform/enquiries/{enquiry}` (audited `enquiry.deleted`) — platform-level audits (school_id null).
  - Web: marketing `EnquiryModal` replaces `RegistrationModal` (removed) — posts via the shared `api` client, shows a success state. Platform **Enquiries** sidebar item → `/platform/enquiries`: list with search/status filter, inline status dropdown (tinted), email/phone quick links, delete-with-confirm, pagination, new-count badge.
  - Backend feature tests in `tests/Feature/Platform/EnquiryTest.php` (5: public submit, validation, platform list+filter+status update+audit, delete, school-user 403). Full suite: **167/167 passing**.
  - Verified with `php artisan migrate`, `php artisan test`, `npm run build` (264 modules; existing chunk-size warning), ESLint clean on all new/changed files (pre-existing FeaturesCarousel lint errors untouched), and a live curl smoke (public submit 201 → validation 422 → platform list shows lead w/ new_count).

Not Started (Phase 4 remaining modules — build order): school onboarding, subscription plans, school subscription assignment, module/feature access control, billing & invoices, platform users & permissions, support tickets, announcements to schools, notification/payment gateway management, platform reports & audit logs.
