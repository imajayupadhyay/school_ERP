...
import { ChevronDownIcon, LogoutIcon, MenuIcon, PanelLeftIcon } from '@/features/admin/components/icons'
...
export default function PlatformTopbar({ collapsed, onOpenMenu, onToggleCollapse }: PlatformTopbarProps) {
  ...
  const onSignOut = async () => {
    await signOut()
    navigate('/schoollid-secure-login', { replace: true })
  }
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[68px] items-center gap-2 border-b border-line bg-paper/80 px-4 backdrop-blur-xl md:gap-3 md:px-6">
      ...
      <div className="hidden sm:flex">
        <span className="inline-flex items-center gap-2 rounded-full border border-line bg-white px-3 py-1.5 text-[0.78rem] font-semibold text-ink/55">
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            ...
            <span className="hidden text-left md:block">
              ...
              <span className="block text-[0.72rem] leading-tight text-ink/50">Super Admin</span>
            </span>
            <ChevronDownIcon
              width={16}
              height={16}
